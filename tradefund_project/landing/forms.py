...
class CustomSignupForm(SignupForm):
    ...
    def save(self, request):
        ...
        if created:
            # For new users, initialize profile with zero balance and no active investment.
            # Actual investment activation and fund allocation will occur after admin approval of payment.
            profile.initial_investment_amount = Decimal('0.00')
            profile.investment_start_date = None  # Will be set upon payment approval
            profile.current_cycle_principal = Decimal('0.00')
            profile.current_cycle_start_date = None  # Will be set upon payment approval
            profile.is_awaiting_reinvestment_action = False # No active cycle yet
            
            profile.current_balance_cached = Decimal('0.00')
            profile.total_earnings_cached = Decimal('0.00')
            profile.save()  # Save all initialized fields for the new profile
        else:
            # If the profile already existed (e.g., edge case),
            # just update the selected tier. Other profile fields related to
            # investment should be managed by subsequent processes (e.g., payment approval).
            profile.save(update_fields=['selected_tier'])
            
        # Initial Transaction, PortfolioSnapshot, and DailyLedgerEntry are NOT created here.
        # This is because the initial investment amount is effectively zero at signup.
        # These records will be created when an admin approves the user's first investment payment
        # and allocates the corresponding tier amount.
...
class ProofOfPaymentForm(forms.ModelForm):
    class Meta:
        model = Transaction
        fields = ['proof_of_payment', 'amount', 'related_tier_key', 'user', 'type', 'status'] # Corrected this line
        widgets = {
            'proof_of_payment': forms.ClearableFileInput(attrs={
                'class': 'form-file-style mt-1 block w-full text-sm text-gray-500 file:mr-4 file:py-2 file:px-4 file:rounded-full file:border-0 file:text-sm file:font-semibold file:bg-secondary file:text-primary hover:file:bg-opacity-90',
                'accept': 'image/*,application/pdf' # Specify accepted file types
            }),
            'amount': forms.HiddenInput(), # Amount will be pre-filled based on tier
            'related_tier_key': forms.HiddenInput(), # Tier key will be pre-filled
        }
        labels = {
            'proof_of_payment': 'Upload Proof of Payment (Image or PDF)',
        }

    def __init__(self, *args, **kwargs):
        self.tier_amount = kwargs.pop('tier_amount', None)
        self.tier_key = kwargs.pop('tier_key', None)
        super().__init__(*args, **kwargs)
        
        # Ensure the fields exist before trying to set initial values or required status
        if 'amount' in self.fields and self.tier_amount is not None:
            self.fields['amount'].initial = self.tier_amount
        if 'related_tier_key' in self.fields and self.tier_key is not None:
            self.fields['related_tier_key'].initial = self.tier_key
        
        # Ensure the proof_of_payment field is required
        if 'proof_of_payment' in self.fields:
            self.fields['proof_of_payment'].required = True


    def clean_proof_of_payment(self):
        proof = self.cleaned_data.get('proof_of_payment', False)
        if proof:
            if proof.size > 5*1024*1024: # 5MB limit
                raise forms.ValidationError("File size must be under 5MB.")
            # You can add more validation for file types if needed, e.g., using Pillow for images
            return proof
        else:
            # This validation will trigger if 'proof_of_payment' is not in cleaned_data
            # which happens if it's not submitted and required=True
            raise forms.ValidationError("Please upload a proof of payment.")
